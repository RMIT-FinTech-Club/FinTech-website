...
const EventCard: React.FC<{ photoInfo: PhotoInfo }> = ({ photoInfo }) => {
  return (
    <img className="w-full h-full rounded-2xl object-cover" src={photoInfo.url} alt={photoInfo.description} />
  );
}
...
const PreArrow: React.FC<{ onClick?: () => void }> = ({ onClick }) => {
  return (
    <div className="absolute top-1/2 left-3 z-10">
      <button onClick={onClick} className="flex justify-center w-10 h-10 rounded-full bg-ft-primary-yellow hover:bg-ft-primary-yellow-600">
...
const EventGallery: React.FC = () => {
  ...
  return (
    <div className="w-full h-auto py-6 bg-ft-background">
      <h1 className="w-full h-full my-4 text-center text-ft-primary-blue text-4xl font-extrabold">Event Recap</h1>
      <div className="w-[16vw] h-0.5 bg-ft-primary-yellow m-auto" />
      <div className='flex justify-center'>
        <Slider className="w-[90vw] md:w-[80vw] lg:w-[60vw] m-2" {...settings}>
