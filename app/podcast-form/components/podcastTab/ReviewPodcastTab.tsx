...
const ReviewPodcastDataTab: React.FC<{ isShow: boolean }> = ({ isShow }) => {
	...
	useEffect(() => {
		...
		console.log(
			`previewAuthorImageRefs.current: ${previewAuthorImageRefs.current}`,
		);
		for (let i = 0; i < authorFiles.length; ++i) {
			console.log(
				`check previewAuthorImagRef at index: ${i}, component: previewAuthorImageRef[${i}]: ${previewAuthorImageRefs.current[i]}`,
			);
									...
									{podcastDetail.authors.map(
										(item: any, index: number) => (
											...
											>
												<img
													ref={(el) => {
														previewAuthorImageRefs.current[
															index
														] = el;
													}}
