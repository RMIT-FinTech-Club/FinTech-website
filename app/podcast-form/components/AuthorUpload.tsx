...
const AuthorUpload = () => {
  const { podcastDetail, setPodcastDetail, authorFiles,  setAuthorFiles } = useContext(PodcastDataContext);
  ...
  function handleAuthorTitle(event: React.ChangeEvent<HTMLInputElement>) {
    podcastDetail.authors[currentAuthorIndex - 1].title = event.target.value;
    setPodcastDetail({ ...podcastDetail });
  }
  return (
    <>
          ...
          >
            ...
            className="py-3 z-10"
              onClick={() => {
                ...
                const modifyPodcastDetailObject = podcastDetail;
                const modifyAuthorFilesObject = authorFiles;
                modifyPodcastDetailObject.authors.splice(index, 1);
                modifyAuthorFilesObject.splice(index, 1);
                setAuthorFiles(modifyAuthorFilesObject);
                setPodcastDetail({ ...modifyPodcastDetailObject});
                }
              }}
            >
              <IoClose />
      ...
      <section className="w-full h-[150px] flex justify-center gap-4">
        ...
        <div className="w-2/3 flex flex-col justify-between">
          <p className="text-md font-semibold">Author {currentAuthorIndex}</p>
          <input
            onChange={handleAuthorName}
            className={inputStyle}
            placeholder="Author Name"
            value={podcastDetail.authors[currentAuthorIndex - 1]?.name}
          />
          <input
            onChange={handleAuthorTitle}
            className={inputStyle}
            placeholder="Author Title"
            value={podcastDetail.authors[currentAuthorIndex - 1]?.title}
          />
        </div>
      </section>
    </>
